-- Fix security leak in get_kid_activity_stats
-- This migration adds an ownership check to ensure the requester owns the kid profile.
...
DECLARE
    ...
    rec RECORD;
BEGIN
    -- SECURITY CHECK: Verify that the authenticated user owns the kid profile
    -- auth.uid() is available because the function is called from the Supabase API/REST
    IF NOT EXISTS (
        SELECT 1 FROM public.kid_profiles 
        WHERE id = p_kid_profile_id AND user_id = auth.uid()
    ) THEN
        RAISE EXCEPTION 'Unauthorized: You do not own this kid profile';
    END IF;
